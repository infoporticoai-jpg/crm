...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res?.error) { toast.error("Invalid email or password"); } else { router.push("/dashboard"); }
    ...
    <div className="flex min-h-screen items-center justify-center bg-gray-50 px-4">
      <div className="w-full max-w-md space-y-8">
        <div className="text-center">
          ...
          <p className="mt-1 text-gray-500">Sign in to your Portico account</p>
        </div>
        <div className="rounded-xl border bg-white p-6 shadow-sm">
          ...
          <div className="relative my-6"><div className="absolute inset-0 flex items-center"><div className="w-full border-t" /></div><div className="relative flex justify-center text-xs uppercase"><span className="bg-white px-2 text-gray-500">or</span></div></div>
        ...
        <p className="text-center text-sm text-gray-500">Don&apos;t have an account? <Link href="/signup" className="text-[#DC7418] hover:underline font-medium">Sign up</Link></p>
      </div>
    </div>
  );
}
